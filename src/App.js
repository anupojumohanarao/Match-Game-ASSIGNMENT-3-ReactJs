...
class App extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.mainStatusChange, 1000)
  }
  ...
  onImageClick = thumbnailUrl => {
    ...
    const imgValues = imagesList.filter(
      eachValue => eachValue.thumbnailUrl === thumbnailUrl,
    )
    const {imageUrl} = imgValues[0]
    if (imageUrl === imgUrl) {
      const newImgUrl =
  ...
  render() {
    const {isTrue, category, time, imgUrl, totalScore} = this.state
    const totalThumbnailList = imagesList.filter(
      ...
      <div className="main-app-container">
        <nav className="main-nav-container">
          <img
            ...
            alt="website logo"
          />
          <ul className="total-score-card-1">
            <li className="score-name">
              <p>
                Score: <span className="orange">{totalScore}</span>
              </p>
            </li>
            <li className="total-score-card-1">
              <img
                src="https://assets.ccbp.in/frontend/react-js/match-game-timer-img.png"
                className="timer-icon"
                alt="timer"
              />
              <p className="orange">{time} sec</p>
            </li>
          </ul>
        </nav>
        <div className="content-div">
          {!isTrue && (
            <div className="main-first-div-container">
              <img src={imgUrl} alt="match" className="match-images" />
              <ul className="total-tab-elements">
                {tabsList.map(eachValue => (
                  <li key={eachValue.tabId}>
                    <button
                      ...
                      onClick={() => this.onClickTabId(eachValue.tabId)}
                      type="button"
                    >
              ...
              <ul className="thumbnail-list">
                {totalThumbnailList.map(eachObject => (
                  <li key={eachObject.id}>
                    <button
                      className="image-button"
                      type="button"
                      onClick={() => this.onImageClick(eachObject.thumbnailUrl)}
                    >
                      <img
